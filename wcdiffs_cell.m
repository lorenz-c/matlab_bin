...
for i = 1:length(dom)
    if i == 1
        ddt{1,3} = (dom(2)*fields{2} - dom(1)*fields{1})/((dom(2)/2 + dom(1)/2));
    elseif i == length(dom)
        ddt{i,3} = (dom(i)*fields{i} - dom(i-1)*fields{i-1})/((dom(i)/2 + dom(i-1)/2));
    else
        ddt{i,3} = (dom(i+1)*fields{i+1} - dom(i-1)*fields{i-1})/((dom(i+1)/2+dom(i)+dom(i-1)/2));
    end    
